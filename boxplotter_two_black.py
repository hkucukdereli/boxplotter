...
plt.figure(figsize=(8,6), facecolor="#000000", dpi= 150)
ax = plt.subplot(111)
...
boxpl = data.boxplot(column= columns, grid= False, return_type= 'dict', patch_artist= True, positions=[1., 1.5, 2.5, 3.], widths= (0.35, 0.35, 0.35, 0.35), showfliers=False, whis= [5, 95])
maxVal = 1.5
...
you = 'must'
if you == 'must':
    topPos1 = np.amax(whiskers[0:4]) + 0.02*maxVal
    topPos2 = np.amax(whiskers[5:8]) + 0.02*maxVal
    ## draw the significant lines
    ax.plot([1., 1.5], [topPos1, topPos1], color= 'w', linewidth= line)
    ax.plot([2.5, 3.], [topPos2, topPos2], color= 'w', linewidth= line)
